...
const cardClass = "rounded-2xl border border-paper/10 bg-charcoal p-6";
const selectClass =
...
export default function Studio() {
  ...
  const handleExport = async () => {
    ...
    try {
      const selected = getJourney(selectedJourneyId);
      if (selected) {
        setExportStatus(`Rendering ${selected.name}…`);
        const blob = await renderJourneyWav(selected, params.mode);
        downloadBlob(blob, `binaural-studio_${selected.id}.wav`);
      } else {
        const minutes = Number.parseFloat(exportMinutes);
        if (!Number.isFinite(minutes) || minutes <= 0 || minutes > 60) {
          setExportStatus("Enter a duration between 1 and 60 minutes.");
          setExportBusy(false);
          return;
        }
        setExportStatus("Rendering audio…");
        const blob = await renderStaticWav(params, minutes * 60);
        downloadBlob(
          blob,
          `binaural-studio_${params.beatFreq}hz_${params.mode}.wav`
        );
      }
    ...
    <div className="grid gap-6 lg:grid-cols-[1fr_360px]">
      ...
      <div className={cardClass}>
        ...
        >
          {(
            ...
            <button
              ...
              className={`rounded-lg px-3 py-2.5 text-sm transition-colors disabled:opacity-40 ${
                params.mode === mode
                  ? "bg-charcoal text-gold"
                  : "text-sage hover:text-paper"
              }`}
            >
              {label} <span className="text-xs text-sage">· {hint}</span>
            </button>
          ))}
        </div>
        ...
        <fieldset disabled={journeyRunning} className="transition-opacity">
          ...
          {/* Sliders */}
          <div className="mt-6 space-y-5 rounded-xl border border-paper/10 bg-ink/60 p-5">
            <div>
        ...
        <button
          type="button"
          onClick={journeyRunning ? () => void getEngine().stop() : handlePlayStop}
          className={`mt-6 w-full rounded-xl py-3.5 font-medium transition-colors ${
            playing
              ? "border border-paper/25 text-paper hover:border-gold hover:text-gold"
              : "bg-gold text-ink hover:bg-gold/90"
          }`}
        >
          {journeyRunning ? "Stop journey" : playing ? "Stop" : "Play"}
        </button>
      ...
      <div className="space-y-6">
        <div className={cardClass}>
          ...
          <p className="mt-2 text-sm leading-relaxed text-sage">
            Guided sessions that ramp the beat frequency over time — settle
            in, and the studio drives.
          </p>
        ...
        <div className={cardClass}>
          ...
          <p className="mt-2 text-sm leading-relaxed text-sage">
            Render the current settings — or the selected journey — to an
          ...
          <div className="mt-4 flex items-center gap-3">
            ...
            <button
              ...
              disabled={exportBusy}
              className="flex-1 rounded-lg border border-paper/25 py-2.5 text-sm text-paper transition-colors hover:border-gold hover:text-gold disabled:cursor-wait disabled:opacity-50"
            >
              {exportBusy ? "Rendering…" : "Download audio"}
            </button>
          </div>
          <p className="mt-2 h-4 text-xs text-sage" role="status">
            {exportStatus}
          </p>
        </div>
      </div>
